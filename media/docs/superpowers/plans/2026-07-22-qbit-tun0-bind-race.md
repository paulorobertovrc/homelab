...
**Extended re-test follow-up (2026-07-22, same day):** A reviewer flagged that Step 3's ~15-20s outage never reached the healthcheck's `interval:60s × retries:4` ≈ 240s failure threshold, leaving outcome (b) (autoheal recycle) and the entrypoint's actual wait-branch unproven. Re-ran with a ~4m15s outage (10:31:27–10:35:42): `qbittorrent` flipped `unhealthy` at 10:35:03 (4th consecutive failed check), `autoheal` fired within 3s ("found to be unhealthy - Restarting container now"), and the recycled container's entrypoint logged **16 consecutive** `[tun0-wait] tun0 sem IPv4; aguardando VPN...` lines (VPN was still down at restart) before proceeding with `[tun0-wait] tun0 pronto; iniciando qBittorrent.` once the VPN was restored — proving both previously-unproven paths in one test. Note: `RestartCount` stayed at 0 through the recycle — expected, since autoheal's `docker restart` over the socket isn't a restart-policy-triggered restart and doesn't increment that counter; the real evidence is autoheal's own log line plus `StartedAt` advancing (`14:20:53Z` → `14:35:10Z`). Post-restore, qBittorrent rebound clean `10.5.0.2%tun0:34124` and went `healthy` within ~13s. Full transcript: `.superpowers/sdd/task-4-report.md` § "Extended re-test (fix round — Scenario 3 rigor)".
